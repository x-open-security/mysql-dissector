...
use std::io::Error;
use crate::{Command, proto::Protocol, payload::Payload, DBType};
...
pub fn new_mysql_packet(payload: &[u8]) -> Option<MySQLPacket> {
    if payload.len() < 5 {
       return None
    }
    ...
    let payload = Payload::new(DBType::MySQL, payload[5..].to_vec());
...
impl Parser {
    ...
    pub fn flush_parse(&mut self) -> Result<(), Error> {
        Ok(())
    }
}
